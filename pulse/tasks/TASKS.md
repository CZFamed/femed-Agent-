...
**产品版本 V1.15.2｜接口契约 v1.1｜媒体库契约 v1.7.2｜全仓测试 354 项全绿。**
（注意：`git tag` 最新只到 `v1.13.0`，1.14.0–1.15.2 四个版本未打标签，见文末「版本管理」。）
...
**当前版本 = V1.15.2**（代码三处已一致，但**标签没跟上**，见下）。三处版本号必须一致，由
...
> ⚠️ **版本漂移重现（2026-09-17 核实）**：`git tag` 最新只到 **v1.13.0**（共 20 个标签），
> 而 `pyproject.toml` / `pulse.__version__` / `CHANGELOG.md` 已到 **1.15.2**——
> 1.14.0 / 1.15.0 / 1.15.1 / 1.15.2 四个版本**都没有打标签**，
> 且 1.15.2（`eb0877d`）尚未推送，`origin/main` 停在 1.15.1（`8216b39`）。
> 这正是 README §5 记录过的同一类漂移：守护测试只比对那三处，看不到 tag。
> 处理方式：补打 `v1.14.0`–`v1.15.2` 四个带注释标签并推送（由 root 执行）。
...
| `v1.14.0` … `v1.15.2` | **缺标签**：四个版本已提交未打标签，另 1.15.2 未推送 |
