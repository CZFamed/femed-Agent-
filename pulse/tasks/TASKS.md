...
**产品版本 V1.17.1｜接口契约 v1.1｜媒体库契约 v1.7.3｜全仓 699 项（696 通过 + 3 项 xfail 钉桩缺陷）。**
（`v1.17.0` 新增 A1/A3 两个域；`v1.17.1` 交付 A6 独立验证并修掉其中一处内容域缺陷。）
...
（spawn 出的 `a4_compliance`、`a5_platform` 都没收到任务正文，只做了只读排查后结束）。
...
- **A6 报告的四条 A3 缺陷待修**（A6 只报告不代改，root 按契约仲裁后修）：
  | 编号 | 级别 | 现象 | 建议修法 |
  | --- | --- | --- | --- |
  | F-1 | 阻断 | 同步平台返回 `published` 时 `dispatching → published` 不在契约 §3.3 迁移表 → 任务卡住 | 先落一次 `publishing` 再落 `published`（不动契约） |
  | F-3 | 高 | 轮询返回可重试错误时走 `pending_finalize → retrying`（无此边）→ 收敛中断，只能等 30 分钟超时 | 轮询期可重试结果按"一次轮询失败"处理：保持 `pending_finalize` 并重排轮询 |
  | F-2 | 高 | A1 与 A3 各自 mint `unified_post_id`，两层幂等防线不是同一个键 | 由 A3 接受上游传入的键（或在 `variants.fields` 持久化后取用） |
  | F-5 | 中 | `_set_schedule_status` 的 `pending → published` 兜底分支必然抛异常（当前不可达） | 与 F-1 一并改：排期经 `publishing` 落 `published` |
