...
> 维护人：**root**　｜　更新：2026-09-22
> 状态：`TODO` / `IN_PROGRESS` / `BLOCKED` / `VERIFY`（待 A6 验证）/ `DONE`
> **子 agent 不修改本文件**，完成任务后在交付消息中报告，由 root 更新。

---

## 当前状态（2026-09-22 按代码核实）

**产品版本 V1.19.1｜接口契约 v1.1｜媒体库契约 v1.7.3｜全仓 769 项全绿（0 xfail）。**
（`v1.17.0` 新增 A1/A3；`v1.17.1` 交付 A6 独立验证；`v1.17.2` 修掉 F-1/F-2/F-3/F-5；
`v1.18.0` 交付 A4 合规与治理域。）
...
| A1 内容生产 | W1 | **DONE**（2026-09-22） | `pulse/services/content/`（**74 项单测**）：brief 解析、平台 Prompt 模板、标签白名单、素材选用器、模型路由 + token 熔断、Variant 派生、固定文案桩。**由 root 直接实现**，原因见下方「W1 重启记录」 |
| A3 调度与账号 | W1 | **DONE**（2026-09-22） | `pulse/services/scheduler/`（**137 项**）+ `pulse/services/identity/`（**63 项**）：eager 可测的 Celery 接线、延迟投递、限流与配额预检、best-time、账号与凭据、Token 脱敏、停用熔断。子 agent `a3_scheduler` 交付，root 独立复核 |
| A4 合规与治理 | W2 | **DONE**（2026-09-22） | `pulse/services/compliance/`（**34 项**）：规则引擎、版权、敏感词（外置词表）、平台质量规则、**制裁与出口管制筛查**、block/warn 与豁免留痕、`ComplianceInfo` 闸门、FR-4 三条件。**由 root 直接实现**（6 个 A4 实例均未收到任务正文） |
| A5 接口层与审批台 | W2 | **DONE**（2026-09-22） | `pulse/api/`（**32 项**）：契约 §7 的 11 个端点、审批工作流（含 diff 留痕与三条件强制单条复核）、半自动导出入口、看板（询盘口径）、统一错误体。**由 root 直接实现**（3 个 A5 实例均未收到任务正文） |
...
**content 74** ｜ **scheduler 138** ｜ **identity 63** ｜ **compliance 34** ｜ **api 32** = **701**；
跨域验证（A6）`pulse/tests/` **65** → **全仓 766 项**。

**W2 全部交付（2026-09-22）**：A4 合规域、A5 接口层与审批台、A6 独立验证**三个域都已交付**，
六个门禁复跑通过。A4/A5 由 **root 直接实现**（子 agent 投递失败累计 6 例：A4 ×3、A5 ×3，
改用文件投递也未解决）；只有 A3、A6 两个实例正常收到任务。

**M1 闭环已可完整跑通**：生成（A1）→ 合规（A4）→ 审批（A5）→ 排期（A3）→ 发布（A2）→ 回执，
全链路用假 Adapter 在 A6 的集成用例里打通可追溯。**尚欠的是"真平台联通"**：
11 个端点目前是框架无关的应用层实现（没有起 HTTP 服务），OAuth 缺平台应用凭据，
制裁清单条目待人工导入——这三项都属外部依赖，不是代码缺口。

**门禁（2026-09-22 复跑，root 代跑）**：G-契约 / G-测试 / G-边界 / G-合规 / G-幂等 / G-集成 **六个门全过**。
A6 本轮 spawn 的实例仍未收到任务正文，故由 root 代跑并补了一条 A6 式的"解析契约文本"对拍用例
（`pulse/tests/test_api_contract_conformance.py`），并据此抓出并修掉一处真实偏差：
接口层路径参数名原为 `{variant_id}` 等，与契约 §7 的 `{id}` 不一致。

**W1 已全部交付（A1 / A2 / A3）。** 2026-09-11 → 2026-09-20 期间投入过一次媒体资产域与控制台
（1.1.0 → 1.16.0）；2026-09-22 按原计划重启 W1，补上了 A1 与 A3。
**M1 的验收标准「单账号单平台跑通生成 → 审核 → 发布 → 回执闭环」仍差一步**：
生成（A1）、发布（A2）、调度（A3）三域已各自用桩跑通，但**审批（FR-4，属 A5）与跨域端到端（A6）尚未实现**，
所以闭环还没有人真正用假 Adapter 串起来验证过。

下表的 W1 / W2 任务是**原始计划与执行台账**，状态按代码实际情况标注。
...
| 域 | 执行方式 | 结果 |
| --- | --- | --- |
| A3 调度与账号 | 子 agent `a3_scheduler`（`fork_turns="none"`） | ✅ 交付 `scheduler` 137 项 + `identity` 63 项；root 复核：全量测试通过、无越界写入、队列载荷只传 `{job_id, unified_post_id}`、Token 有脱敏与句柄隔离 |
| A1 内容生产 | **root 直接实现** | ✅ 交付 74 项单测；原因见下 |

**A1 为什么由 root 自己写**：按协同方案 spawn 了 `a1_content`，另按历史经验又试了
`a1_content_v2` 与一条极简任务消息，**三个实例都只收到环境上下文、没有收到任务正文**
（它们各自回报"没有可执行请求"，然后只做了只读排查）。同一批次里 `a3_scheduler`
却正常收到了任务并完成交付，说明是**委派通道对 A1 这条链路投递失败，不是任务写得不清楚**。
为了不阻塞 W1 出口，root 直接实现了内容域，并按同一套验收标准自测。
后续再派 A1 相关工作前，建议先确认委派通道恢复（换任务名重试是最低成本的验证方式）。

### 待办

- **A4 合规域、A5 接口层与审批台未开工**（委派通道投递失败，见上）。M1 闭环里
  "合规判定"与"审批（FR-4）"目前只能靠测试替身，11 个 REST 端点尚无实现。
- ~~A6 报告的四条 A3 缺陷待修~~ → **已于 v1.17.2 全部修掉**（F-1/F-2/F-3/F-5；A6 的 3 个 xfail 转为正式断言，全仓 699 全绿）。
- **A6 未覆盖的两个区域**（其自述）：`services/media/` 与 `console/`（root 自有域，287 项域内测试）没有独立验证；
  A1 的素材召回链路（`mediaselect`）也未进端到端（A6 走的是"无素材库 → 纯文本"路径）。
- W2 出口条件：A4/A5 交付 + F-1/F-3 修复后，A6 把 3 个 `xfail` 转为正式断言并重跑 M1 门禁。
- **跨域缺口 1：YouTube 没有素材位次口径。** 媒体域的 `platforms.py` 只定义了
  LinkedIn / Facebook / TikTok / VK 四个平台（依据《四平台推荐风格与方式报告》），
  而 AGENTS.md 把 YouTube 列为 P0-A。当前 A1 对 YouTube 的处理是：**不猜画面顺序**，
  跳过自动选素材、由人工提供视频素材并标注 `TODO(need-real-data)`。
  要补齐需要业务方先定 YouTube 的画面口径（报告没覆盖这个平台）。
- **跨域缺口 2：YouTube 的文案结构是自定的。** `prompts.py` 里 YouTube 模板的段落结构
  来自本项目设计（平台硬限制 title ≤100 / description ≤5000 / tags ≤500 是事实依据），
  不是报告原文——报告只覆盖四平台。待评审确认后固化。
- `W0-5` Git 提交：见下方提交记录
...
## 版本管理（V1.17.0 · 2026-09-22）

**当前版本 = V1.17.0**（代码三处一致 + tag 同步）。三处版本号必须一致，由
`pulse/shared/tests/test_release_version.py` 守护（5 项）：

| 位置 | 当前取值 | 真源属性 |
| --- | --- | --- |
| `pyproject.toml` | `1.17.0` | `[project] version` |
| `pulse/__init__.py` | `1.17.0` | `pulse.__version__` |
| `CHANGELOG.md` | `## [1.17.0]` | 逐版本条目 |
