...
> 所有者：root　｜　冻结日期：2026-09-11　｜　最近升版：2026-09-14（v1.7）　｜　实现：`pulse/services/media/`
...
- 触发时点：素材**实际进入生成内容或发布**时调用 `RecallLedger.mark_used`；
  仅被检索命中不写台账、不消耗冷却。
- **导出即触发（v1.6 明确）**：`POST /api/export` 成功落盘后，包内**真的复制成功**的素材
  立即写入台账并开始计时；没进包的素材不记，避免白扣 15 天。
  导出 = 这一帖的素材已定稿并交给人工发布，属"进入发布"。
...
- 入库仅接受**实拍**素材：图片 `.jpg/.jpeg/.png/.webp/.bmp`（≤ 20 MB，`MAX_IMAGE_BYTES`）；
  ...
  设计稿 `.psd/.psb`（≤ 200 MB，`MAX_PSD_BYTES`，见 §2.5.3）。
  工业件禁止文生图产品图（AGENTS.md §3.6）。
- 同一内容哈希重复入库一律拒绝，返回 `DuplicateMediaError`。
- **同名描述已存在时同样拒绝**：既有素材（老图）不在登记表里，
  光靠内容哈希查不出来；直接写下去会覆盖原描述，属不可逆的数据损失。

### 2.5.2 先查重再识别（B5.2，v1.5 新增）

- **识别前必须先做内容查重**，命中即直接返回"已在库中"，**不调用模型**。
  动机：重复素材原本要到入库时才被拒，但那时的识别费已经花掉
  （图片约 2,000 token，视频约 3,000 token）。
- 查重范围 = **登记表（控制台入库的素材）+ 既有素材目录（老图）**，
  按**内容哈希**判定，与文件名无关。
- 性能口径：素材库约 1.34 GB / 336 个文件，**不允许每次上传都全库重算哈希**。
  实现为**先按字节数过滤**（`stat` 几百个文件仅数毫秒），
  只对尺寸相同的极少数文件真算哈希——实测单次查重 **38–73 ms**。
- 浏览器侧用 `crypto.subtle` 先算 SHA-256 调 `/api/precheck`：
  命中则**连文件都不上传**（对 200 MB 视频尤其有意义）。
  浏览器不支持时自动跳过，服务端在 `/api/describe` 里**再查一次**（权威判定）。
...
### 2.5.3 PSD 设计稿（B5.3，v1.7 新增）

- **白名单**：`.psd` / `.psb` 按图片处理（`.psb` 是 Photoshop 的大文档格式），
  体积上限单独给 `MAX_PSD_BYTES`（200 MB）——设计稿比实拍图大得多，
  仍用 20 MB 会把人挡在门外。
- **为什么要转**：视觉接口只吃栅格图，浏览器也不认识 Photoshop 文档，社媒更发不出去。
  因此入库/预览/导出都把 PSD 的**合并图（composite）**解出来再编码成 PNG
  （`pulse/services/media/psd.py`，纯标准库，不引第三方依赖）。
- **支持子集**：PSD(1) 与 PSB(2)；色彩模式 位图 / 灰度 / 索引色 / RGB / CMYK；
  位深 8 / 16（16 位取高字节）；压缩 原始 与 RLE(PackBits)；有 alpha 通道时输出带透明 PNG。
  **不支持** 32 位浮点、ZIP 压缩、多通道 / 双色调 / Lab——遇到这些一律抛
  `UnsupportedPsdError` 并给出中文原因，**绝不悄悄给一张错图**。
- **零修饰口径仍然成立**：转换只做"解码 + 可选整数倍降采样"，不裁剪、不调色、不加字。
  送模型的 PNG 最长边 1600 px（`psd.DEFAULT_MAX_SIDE`），页面预览同样；
  导出时按原分辨率转出，并且**原 `.psd` 与转换出的 `.png` 一起进包**，
  在 `说明.txt` 里逐条注明"PSD 解出的合并图 → PNG，未做任何修饰"。
- **描述命名**：PSD 属图片，描述文件仍是 `<文件名去扩展>.md`；`content_type: "图片描述"`。
- **降级**：解不出来时不影响原文件——导出仍按字节复制原 PSD，只在说明里写"转换 PNG 失败"。
...
- **导出即计冷却（v1.6）**：导出成功后把包内素材写入台账，`content_id` = 导出目录名，
  同一次导出重复调用幂等；返回值多出 `used`（本次新计入的素材 ID）与 `cooldown_days`，
  界面提示"M 张素材进入 15 天冷却"并立即刷新冷却计数。
...
| POST | `/api/export` | 导出素材包（`platform`、`slots`[{order,role,asset_id}]、可选 `caption`）；返回落盘目录，并把包内素材记入冷却（`used`、`cooldown_days`） |
| POST | `/api/precheck` | 上传前查重（`hash`、可选 `size`）；命中返回 `duplicate: true` 与已有素材信息 |
...
| 1.5 | 2026-09-11 | 新增 B5.2 先查重再识别（登记表 + 既有库、按内容哈希、字节数过滤保证 38–73ms、浏览器侧预查重）；B5 补"同名描述拒绝覆盖"；控制台接口表补 `/api/precheck` |
