...
- 品类的 `sub_process` **可为空**：库里"人员"这类品类本来就没有子目录。
  **空子类必须一路透传**：控制台不得把空值改写成"未分类"（那会让用户手选的品类
  在品类清单里对不上号、被判成"没判断出品类"），前端也不得把子类下拉设成必选
  （空值会被浏览器判成"未选择"，直接拦下整张表单——2026-09-15 的实际 BUG）。
...
#### 2.9.1 位次补足阶梯（B9.1，v1.7.3 新增）

**背景**：四道闸（品类 → 关键词 → 同档 95% → 冷却）叠加后，2026-09-20 实测
"库里 468 张图，LinkedIn 四个位次只出得来 1 张"——同档阈值把某个位次的
157 张候选砍到 4 张、冷却再砍到 0。**"一堆图却一张也召回不来"是缺陷，不是设计。**

- **先精准、再逐级放宽，保证数量**。每个位次按下面的阶梯依次取，
  够数就停；候选池按"档位"分批、每轮**所有位次都先跑一遍**，
  避免靠前的位次用放宽档把靠后位次唯一匹配的那张图抢走。

  | 档位 | 候选池 | 页面标注 |
  | --- | --- | --- |
  | `strict` | 顶层品类 + 关键词命中 + 同档 95% | 精准匹配 |
  | `same_category` | 顶层品类 + 关键词命中（去掉同档限制） | 放宽：同品类（去掉同档限制） |
  | `category_any_keyword` | 该品类全部素材（不限关键词） | 放宽：同品类（不限关键词） |
  | `whole_library` | 全库素材 | 放宽：全库 |

- **放宽必须可见**：每个位次返回 `level`、`level_label`、`relaxed`，
  页面明确写出"这一格没有精准素材，这张是放宽补位，建议补拍"，
  不允许把补位图当成精准匹配展示。
- **同一条素材不在同一帖里重复占用两个位次**；跨位次取用按档位分批，保证公平。
- **冷却自始至终不让路**（B1/B12）：进了冷却就是 15 天，不因为凑数而破例。
  某个位次全在冷却期时仍然为空，并置 `blocked_by_cooldown = true`。
- **数量对不上要如实报**：响应新增 `requested`（要求张数，= 每格条数 × 位次数）
  与 `shortfall`（缺几张）。`shortfall > 0` 只可能是素材真的不够
  （库太小或冷却是全部候选），页面据此提示补拍，不再静默少给。
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| 1.7.3 | 2026-09-20 | 新增 B9.1 位次补足阶梯：原先"挑不到就留空"叠加同档阈值与冷却后，468 张图里 LinkedIn 四个位次只出得来 1 张；现改为"精准 → 同品类 → 全库"逐级放宽补足数量（按档位分批、各位次公平取用、不重复占用），放宽档位必须回传 `level`/`relaxed` 并在页面标出；冷却仍不让路；新增 `requested`/`shortfall` 如实报缺 |
