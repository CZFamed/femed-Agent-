...
**当前版本：V1.10.1**（接口契约 v1.1 / 媒体库契约 v1.3）
...
| 项 | 取值 | 真源 |
| --- | --- | --- |
| 产品版本 | **1.10.1** | `pyproject.toml` + `pulse.__version__` |
...
git show v1.10.1 --stat --no-patch
```
...
3. 提交、打标签、推送：
   ...
   ```powershell
   git commit -am "release: V1.10.1"
   git tag -a v1.10.1 -m "V1.10.1 — 禁用态按钮可读性与导出按钮位置"
