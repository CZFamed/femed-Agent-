...
**当前版本：V1.10.2**（接口契约 v1.1 / 媒体库契约 v1.3）
...
| 项 | 取值 | 真源 |
| --- | --- | --- |
| 产品版本 | **1.10.2** | `pyproject.toml` + `pulse.__version__` |
...
git show v1.10.2 --stat --no-patch
```
...
3. 提交、打标签、推送：
   ...
   ```powershell
   git commit -am "release: V1.10.2"
   git tag -a v1.10.2 -m "V1.10.2 — 视觉失败给出真因 + 自检按钮"
